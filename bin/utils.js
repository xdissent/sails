module.exports = function(sails) {
	...
	function UtilityBelt() {
		...
		this.generateDir = function(newPath, gitkeep) {
			if (!newPath) {
				sails.log.verbose('Creating directory in pwd...');
			} else {
				sails.log.verbose('Creating directory directory in ' + newPath + '...');
			}
		...
		this.copySails = function(destination, cb) {
			...
			sails.log.verbose('Copying modules into ' + destination + '...');
			var canvas = turtle();
		...
		this.isValidECMA51Variable = function(v) {
		...
		this.trimSlashes = function(str) {
			return require('underscore.string').trim(str, '/');
		};
