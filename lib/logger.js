module.exports = function(sails) {
    ...
    function Logger(config) {
        ...
        function CaptainsLog (prefix) {
            // Basic log usage
            var clog = log('debug', prefix);
            clog.log = log('debug', prefix);
            clog.debug = log('debug', prefix);
            clog.verbose = log('verbose', prefix);
            clog.info = log('info', prefix);
            clog.warn = log('warn', prefix);
            clog.error = log('error', prefix);
            clog.namespace = function (name) {
                return CaptainsLog(name + ':');
            };

            // ASCII art and easter eggs
            clog.ship = drawShip(clog);
            return clog;
        }
